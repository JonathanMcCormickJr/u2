...
impl U2f {
    ...
    pub fn register_response(
        ...
        if elapsed_time(challenge.timestamp) > TimeDelta::seconds(300) {
            return Err(U2fError::ChallengeExpired);
        }

        let registration_data: Vec<u8> =
            decode_config(&response.registration_data[..], URL_SAFE_NO_PAD).unwrap();
    ...
    pub fn sign_response(
        ...
        let client_data: Vec<u8> = decode_config(&sign_resp.client_data[..], URL_SAFE_NO_PAD)
            .map_err(|_e| U2fError::InvalidClientData)?;
        let sign_data: Vec<u8> = decode_config(&sign_resp.signature_data[..], URL_SAFE_NO_PAD)
